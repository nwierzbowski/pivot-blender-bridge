...
def _get_and_enforce_root_collection():
    ...
    if pivot_root:
        ...
        _ensure_surface_collections_exist(pivot_root)
    
    return pivot_root

def get_or_create_surface_collection(pivot_root, str surface_key):
    ...
    collection_name = _get_surface_display_name(surface_key)
    
    # Try to reuse existing collection
    existing = bpy.data.collections.get(collection_name)
    if existing:
        collection_manager.ensure_collection_link(pivot_root, existing)
...
def collect_group_classifications() -> dict:
...
def organize_group_into_surface(group_collection, str surface_key, pivot_root) -> None:
    ...
    surface_coll = get_or_create_surface_collection(pivot_root, surface_key)
    if not surface_coll:
        return
